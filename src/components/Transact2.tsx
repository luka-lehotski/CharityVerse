...
const Transact2 = ({ openModal, setModalState }: TransactInterface) => {
    ...
    <dialog id="transact_modal" className={`modal ${openModal ? 'modal-open' : ''} bg-slate-200`} style={{ display: openModal ? 'block' : 'none' }}>
      <form method="dialog" className="modal-box">
        ...
        <input
          type="number"
          data-test-id="amount"
          placeholder="Provide amount: (algo)"
          className="input input-bordered w-full"
          value={receiverAmount}
          onChange={(e) => {
            setReceiverAmount(parseInt(e.target.value))
          }}
        />
        <div className="modal-action grid">
          ...
          >
            {loading ? <span className="loading loading-spinner" /> : 'Send your Donation'}
          </button>
        </div>
      </form>
    </dialog>
